...
# for h 10 k 2 create a feasible problem
tp.rwtargets
tp.rwtargets[1]
...
wh = tp.wh
# scale = 1e3
scale = sum(abs.(tp.xmat), dims=1) ./ size(xmat)[1] 
...
# fprob = Optimization.OptimizationProblem(fpof, wh0, lb=zeros(length(wh0)), ub=ones(length(wh0)))
fprob = Optimization.OptimizationProblem(fpof, ratio0, lb=0.1*ones(length(ratio0)), ub=10*ones(length(ratio0)))
algorithm=:(LD_CCSAQ())
...
opt = Optimization.solve(fprob, NLopt.eval(algorithm), maxiters=100, reltol=1e-16)
